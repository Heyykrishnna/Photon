...
export function useCampaignMetadata() {
  return useQuery<CampaignMetadata>({
    ...
    queryFn: async () => {
      const { data } = await apiClient.get<any>("/sa/campaigns/metadata");
 ...
 * Campaign detail
 */
export function useCampaignDetail(campaignId?: string, queryOptions: any = {}) {
  return useQuery<Campaign | undefined>({
    queryKey: queryKeys.campaigns.detail(campaignId ?? ""),
    queryFn: async () => {
      const { data } = await apiClient.get<any>(`/sa/campaigns/${campaignId}`);
      return data?.data;
    },
    enabled: !!campaignId,
    ...queryOptions,
...
export function useScheduleCampaign() {
  ...
  return useMutation<any, Error, { campaignId: string; scheduledAt: string }>({
