...
export function useClub(clubId?: string) {
  return useQuery<NormalizedClub | null>({
    ...
    queryFn: async () => {
      const { data } = await apiClient.get<any>(`/sa/clubs/${clubId ?? ""}`);
...
export function useUpdateClub() {
  ...
  return useMutation<Club, Error, { clubId: string } & Partial<Club>>({
...
export function useRemoveUserFromClub() {
  ...
  return useMutation<void, Error, { clubId: string; userId: string }>({
