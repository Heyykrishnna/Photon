...
export function useMarkCompetitionAttendance() {
  ...
  return useMutation<any, Error, { competitionId: string; userId: string }>({
...
export function useMarkFestAttendance() {
  ...
  return useMutation<any, Error, { userId?: string; qrData?: string }>({
...
export function useSearchParticipantsWithComp(
  ...
  return useQuery<Participant[]>({
    queryKey: [
      ...queryKeys.attendance.participants(query ?? ""),
      competitionId ?? "",
    ],
    queryFn: async () => {
      const { data } = await apiClient.get<any>(
...
export function useRegistrationDeskVolunteers() {
  return useQuery<Volunteer[]>({
    ...
    queryFn: async () => {
      const { data } = await apiClient.get<any>("/volunteer/registration-desk");
